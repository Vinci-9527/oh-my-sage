...
**两种使用方式**：
1. 🌐 **Web 方式** - 基于浏览器的交互界面，适合日常使用  
2. 🔌 **MCP 方式** - 通过 Model Context Protocol 集成到 AI 编程助手，适合开发场景
...
**Step 3: 安装 Skill（⚠️ 创建规则必需）**

Skill 位于 `.agents/skills/mijia-automation/` 目录下：

- **OpenCode**: 复制到 `~/.config/opencode/skills/`，然后对话中说"激活 mijia-automation skill"
...
3. **创建自动化规则**（⚠️ 必须先激活 skill）：
   ```
   请先激活 mijia-automation skill，然后帮我创建一个自动化规则
   ```
...
### ⚠️ 重要：创建/更新规则前必须先激活 Skill

`mijia_create_graph` 和 `mijia_update_graph` 工具描述中明确要求：

> 在调用此工具前，必须先激活 `mijia-automation` skill 以获取规则创建指南。
...
**正确的工作流程**：
1. 激活 skill → 2. 理解规则 → 3. 构建 nodes → 4. 校验 → 5. 创建/更新

**如何激活 skill**：
- **OpenCode**: 直接说 "激活 mijia-automation skill"
- **Claude Desktop / Claude Code / Cursor**: 将 skill 内容添加到对话上下文或 AGENTS.md
