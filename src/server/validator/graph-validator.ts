...
export function validateGraph(graph: Graph): ValidationError[] {
  ...
  for (const node of graph.nodes) {
    ...
    if (!node.type) { errors.push({ nodeId: node.id, type: 'missing_type', level: 'error', message: '缺少 type' }); continue; }
    ...
    if (node.type === 'condition') {
      ...
      if (!incoming.has(`${node.id}.condition`) || incoming.get(`${node.id}.condition`)!.size === 0) {
        errors.push({ nodeId: node.id, type: 'cond_no_condition', level: 'error', message: `condition "${node.id}" condition 无信号来源（需 state 节点 output 连接，如 timeRange.output）` });
      }
    }
...
if (typeof require !== 'undefined' && require.main === module) {
  ...
  if (!fp) { console.error('用法: npx tsx src/server/validator/graph-validator.ts <graph-json>'); process.exit(1); }
